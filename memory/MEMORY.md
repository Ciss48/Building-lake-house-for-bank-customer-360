...
- [final_done.md](final_done.md) — Final (xong 2026-06-10): README tiếng Việt + business_value.md + **push GitHub (repo Ciss48/Building-lake-house-for-bank-customer-360)**. Dự án HOÀN CHỈNH. Còn tuỳ chọn: scrub dev creds + fix double-count snapshot_date
- [nessie_restart_after_docker_stop.md](nessie_restart_after_docker_stop.md) — **GOTCHA OPS**: sau khi tắt/bật Docker, Nessie 0.79.0 fail mọi POST commit (Vert.x NPE `this.pending` null → "EOF/header parser received no bytes" phía Spark); đọc vẫn OK. Fix: `docker restart lakehouse-nessie` rồi chạy lại (không mất data, store ROCKSDB)
